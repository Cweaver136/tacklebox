...
exports.isNodeFlag = (flag, bareword = true) => {
  ...
  return (// treat --require/-r as Mocha flag even though it's also a node flag
    !(flag === 'require' || flag === 'r') && ( // check actual node flags from `process.allowedNodeEnvironmentFlags`,
    // then historical support for various V8 and non-`NODE_OPTIONS` flags
    // and also any V8 flags with `--v8-` prefix
    nodeFlags.has(flag) || debugFlags.has(flag) || /(?:preserve-symlinks(?:-main)?|harmony(?:[_-]|$)|(?:trace[_-].+$)|gc(?:[_-]global)?$|es[_-]staging$|use[_-]strict$|v8[_-](?!options).+?$)/.test(flag))
  );
};
/**
...
exports.unparseNodeFlags = opts => {
  ...
  return args.length ? args.join(' ').split(/\b/).map((arg, index, args) => arg === ' ' && args[index - 1] !== 'require' ? '=' : arg).join('').split(' ') : [];
};