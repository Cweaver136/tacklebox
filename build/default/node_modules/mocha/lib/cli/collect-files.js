...
module.exports = ({
  ...
  spec.forEach(arg => {
    ...
    if (typeof newFiles !== 'undefined') {
      ...
      newFiles = newFiles.filter(fileName => ignore.every(pattern => !minimatch(fileName, pattern)));
    }

    files = files.concat(newFiles);
  });
  const fileArgs = file.map(filepath => path.resolve(filepath));
  files = files.map(filepath => path.resolve(filepath)); // ensure we don't sort the stuff from fileArgs; order is important!
  ...
  if (!files.length) {
    ...
    const noneFoundMsg = unmatched.length === 1 ? `Error: No test files found: ${JSON.stringify(unmatched[0].pattern)}` // stringify to print escaped characters raw
