...
function Mocha(options) {
  ...
  if ('useColors' in options) {
    utils.deprecate('useColors is DEPRECATED and will be removed from a future version of Mocha. Instead, use the "color" option');
  ...
  this.grep(options.grep).fgrep(options.fgrep).ui(options.ui).bail(options.bail).reporter(options.reporter, options.reporterOptions).useColors(options.color).slow(options.slow).useInlineDiffs(options.inlineDiffs).globals(options.globals);

  if ('enableTimeouts' in options) {
    utils.deprecate('enableTimeouts is DEPRECATED and will be removed from a future version of Mocha. Instead, use "timeout: false" to disable timeouts.');
  ...
  ['allowUncaught', 'asyncOnly', 'checkLeaks', 'delay', 'forbidOnly', 'forbidPending', 'fullTrace', 'growl', 'invert'].forEach(function (opt) {
    if (options[opt]) {
      this[opt]();
    }
  }, this);
  ...
  if (typeof reporter === 'function') {
    ...
    if (!_reporter) {
      try {
        ...
        if (err.code !== 'MODULE_NOT_FOUND' || err.message.indexOf('Cannot find module') !== -1) {
          // Try to load reporters from a path (absolute or relative)
          try {
            _reporter = require(path.resolve(process.cwd(), reporter));
          } catch (_err) {
            _err.code !== 'MODULE_NOT_FOUND' || _err.message.indexOf('Cannot find module') !== -1 ? console.warn(sQuote(reporter) + ' reporter not found') : console.warn(sQuote(reporter) + ' reporter blew up with error:\n' + err.stack);
          }
        } else {
          console.warn(sQuote(reporter) + ' reporter blew up with error:\n' + err.stack);
        }
      }
    }
...
Mocha.prototype.globals = function (globals) {
  this.options.globals = this.options.globals.concat(globals).filter(Boolean).filter(function (elt, idx, arr) {
