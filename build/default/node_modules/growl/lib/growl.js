'use strict'; // Growl - Copyright TJ Holowaychuk <tj@vision-media.ca> (MIT Licensed)
...
function growl(msg, opts, callback) {
  ...
  if (image || options.image) {
    ...
    switch (cmd.type) {
      ...
      case 'Linux':
        args.push(cmd.icon, image); // libnotify defaults to sticky, set a hint for transient notifications
  ...
  if (options.sticky && cmd.type === 'Linux') args.push('0'); // priority
    ...
    case 'Custom':
      {
        ...
        const message = options.title ? `${options.title}: ${msg}` : msg;
